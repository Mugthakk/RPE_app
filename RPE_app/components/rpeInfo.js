...
export default class RPEInfo extends Component {
  ...
  render(){
    return(
    <ScrollView style={{backgroundColor: "grey", paddingHorizontal: 10, paddingTop: 30, paddingBottom: 50}}>

      <Button marginHorizontal={20} title="RPE to %" color="red" onPress={() => {this.setState({"rpeChartVisible": true})}}/>
      ...
      <Button marginHorizontal={20} title="RPE estimation chart" color = "red" onPress={() => {this.setState({"rpeEstimationVisible": true})}}/>
      ...
      onRequestClose={ () => {this.setState ( {"rpeEstimationVisible": false} )} }>
        <TouchableWithoutFeedback onPress={() => {this.setState ( {"rpe>EstimationVisible": false} )} }>
